...
const DepartmentReportPage = () => {
  const [departmentReportData, setDepartmentReportData] = useState<
    DepartmentReport[]
  >([]);
  ...
  const handleSearch = () => {
    if (searchQuery) {
      const filtered = departmentReportData.filter((report) =>
        report.departmentName?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    ...
    <div className='container mx-auto px-4 py-8'>
      ...
      <div className='mb-4 flex'>
        <input
          ...
          placeholder='Search'
          className='p-2 border border-gray-300 rounded-l w-full md:w-1/2 lg:w-1/3 focus:outline-none focus:ring-2 focus:ring-blue-500'
        />
        <button
      ...
      <table className='min-w-full border border-gray-300 dark:border-gray-700'>
        ...
        <tbody>
          {filteredData.length === 0 ? (
            <tr>
              <td
                className='text-center py-4 border border-gray-300 dark:border-gray-700'
                colSpan={6}
              >
                No data available
              </td>
            ...
            filteredData.map((report, index) => (
              <tr
                key={index}
                className='bg-white dark:bg-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'
              >
