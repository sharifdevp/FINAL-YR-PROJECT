...
const EditProfile = ({ user, onUpdateImage }: EditUserProps) => {
  ...
  const formSchema = z.object({
    phone: z.string().nonempty('Phone number is required.').max(50),
    birthName: z.string().nonempty('Staff name is required.').max(50),
  });
  ...
  async function SubmitEditUser(values: z.infer<typeof formSchema>) {
    const id = user.id;
    const imageToUpdate = newImage ?? user.image ?? '';
    try {
      const res = await fetch('/api/user-profile/profileId', {
        method: 'PATCH',
        body: JSON.stringify({ ...values, id, image: imageToUpdate }),
      });

      if (res.ok) {
        toast.success('Profile updated', { duration: 9000 });
        onUpdateImage(user.id, imageToUpdate);
        setOpen(false);
        router.refresh();
      } else {
      ...
      <Form {...form}>
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Staff Name (in full)</FormLabel>
                <FormControl>
                  <Input
                    placeholder='Enter your First and Given Name'
                    {...field}
                  />
                </FormControl>
                <FormMessage />
